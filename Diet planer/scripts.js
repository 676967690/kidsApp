// ========== GAME LOGIC with competition mode and fixed event listeners ==========
const FOOD_DB = [
...
function setupGlobalDragDrop() {
    const plateEl = document.getElementById('plateCanvas');
    // Drop target for mouse drag
    plateEl.addEventListener('dragover', (e) => { e.preventDefault(); e.dataTransfer.dropEffect = "copy"; });
    plateEl.addEventListener('drop', (e) => {
        e.preventDefault();
        if (gameWon) { feedbackDiv.innerHTML = "Game completed! Press 'New Level'."; return; }
        const foodIdx = e.dataTransfer.getData("text/plain");
        if (foodIdx === "") return;
        const idx = parseInt(foodIdx);
        if (isNaN(idx)) return;
        if (usedPortionsSum() >= totalParts) {
            feedbackDiv.innerHTML = "❌ Plate is full! Remove some portions first.";
        } else {
            adjustPortion(idx, 1);
        }
        // Clear the data to prevent re-use
        e.dataTransfer.clearData();
    });
    // Plate drag-to-remove (mouse + touch)
    plateEl.addEventListener('mousedown', handlePlateDragStart);
    window.addEventListener('mousemove', handlePlateDragMove);
    window.addEventListener('mouseup', handlePlateDragEnd);
    plateEl.addEventListener('touchstart', handlePlateDragStart, { passive: false });
    window.addEventListener('touchmove', handlePlateDragMove, { passive: false });
    window.addEventListener('touchend', handlePlateDragEnd);
}
// Call once when page loads
setupGlobalDragDrop();

// ========== MODIFIED: changeTotalParts (learning mode now correctly regenerates) ==========
function changeTotalParts(delta) {
    ...
    if (currentMode === 'learning') {
        // Learning mode: completely new puzzle with the new totalParts
        gameWon = false;
        ...
        feedbackDiv.innerHTML = `Learning Mode: New puzzle with ${totalParts} parts. Drag food to plate.`;
    } else {
        // Competition mode: keep same target, only update UI and redraw
...
function resetLevel() {
    ...
    if (currentMode === 'competition') {
        ...
        updateNutrientTable();
        checkWinCondition();
        feedbackDiv.innerHTML = "Competition: Find correct parts & portions. Hint reveals secret parts.";
    } else {
        // Learning mode: random totalParts (3-12) and new puzzle
        totalParts = getRandomInt(3, 12);
        selectedFoods = pickRandomFoods();
        const { target } = generateSolvableTarget(selectedFoods, totalParts);
        targetNutrition = target;
        ...
        updateNutrientTable();
        checkWinCondition();
        feedbackDiv.innerHTML = "Learning Mode: Change parts to generate new puzzle. Drag food to plate.";
    }
    const modal = document.getElementById('challengeModal');
    if (modal) modal.style.display = 'none';
}
...
function showFinalChallenge() {
    ...
    const modalInputs = document.getElementById('modalFoodInputs');
    modalInputs.innerHTML = '';
    selectedFoods.forEach((food, idx) => {
        const div = document.createElement('div');
        div.innerHTML = `<strong>${food.name}</strong> : <input type="number" id="pInp_${idx}" value="${portions[idx]}" min="0" max="${totalParts}" step="1" style="width:70px"> parts`;
    ...
    newSubmit.onclick = () => {
        ...
        let ok = true;
        for (let i = 0; i < selectedFoods.length; i++) {
            const val = parseInt(document.getElementById(`pInp_${i}`).value);
            if (isNaN(val) || val < 0 || val > totalParts) ok = false;
            user.push(val);
        }
        if (!ok || user.reduce((a, b) => a + b, 0) !== totalParts) {
            document.getElementById('modalFeedback').innerHTML = "❌ Sum must equal total parts";
            return;
        }
        if (!user.every((v, i) => v === portions[i])) {
            document.getElementById('modalFeedback').innerHTML = "❌ Portions don't match";
            return;
        }
        const ratioStr = document.getElementById('ratioInput').value.trim();
        const ratioParts = ratioStr.split(':').map(s => parseInt(s.trim()));
        if (ratioParts.length !== 3 || ratioParts.some(isNaN)) {
            document.getElementById('modalFeedback').innerHTML = "❌ Enter ratio a:b:c";
            return;
        }
        const gcd = (a, b) => b === 0 ? a : gcd(b, a % b);
        let g = portions[0];
        for (let i = 1; i < portions.length; i++) g = gcd(g, portions[i]);
        const simplified = portions.map(p => p / g);
        if (JSON.stringify(simplified) !== JSON.stringify(ratioParts)) {
            document.getElementById('modalFeedback').innerHTML = `❌ Correct ratio: ${simplified.join(':')}`;
            return;
        }
        document.getElementById('modalFeedback').innerHTML = "🎉 Perfect! You mastered ratios! 🎉";
        setTimeout(() => {
            modalDiv.style.display = 'none';
            feedbackDiv.innerHTML = "🏆 Great job! Press 'New Level' to play more!";
        }, 1500);
    };
    document.getElementById('closeModalNoValidate').onclick = () => { modalDiv.style.display = 'none'; };
}
...
function startGame(mode) {
    ...
    if (mode === 'competition') {
        ...
        targetNutrition = target;
        portions = [0, 0, 0];
        feedbackDiv.innerHTML = "🏆 COMPETITION: Fixed target! Find correct parts count & portions. Use hint if needed.";
    } else {
        totalParts = getRandomInt(3, 12);
        selectedFoods = pickRandomFoods();
        const { target } = generateSolvableTarget(selectedFoods, totalParts);
...
// ========== KNOWLEDGE SCREEN DRAWING FUNCTIONS ==========
...
// Create challenge modal if missing
